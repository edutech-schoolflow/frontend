    ...
    <AuthShell>
      <div className="mx-auto w-full max-w-[526px] lg:mx-0">
        <div className="flex flex-col gap-[18px]">
          <div>
            <h2 className="text-[24px] font-medium text-[#1b1b1b]">Forgot your password?</h2>
            <p className="mt-[6px] text-[15px] text-[#666]">
              Enter your phone number and we&apos;ll text you a reset code.
            </p>
          </div>

          <form className="mt-[14px] flex flex-col gap-[17px]" onSubmit={submit}>
            ...
            <button type="submit" disabled={busy || !phone} className={`mt-[10px] ${AUTH_BUTTON}`}>
              {busy ? (
                <>
...
export function UnifiedResetPassword({ phone }: { phone: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      toast.success(await resetIdentityPassword({ phone, code: code.trim(), newPassword: password }));
  ...
  return (
    <AuthShell>
      <div className="mx-auto w-full max-w-[526px] lg:mx-0">
        <div className="flex flex-col gap-[18px]">
          <div>
            <h2 className="text-[24px] font-medium text-[#1b1b1b]">Reset your password</h2>
            <p className="mt-[6px] text-[16px] font-medium text-[#1b1b1b]">
              Enter the code we sent{phone ? ` to ${phone}` : ""} and choose a new password.
            </p>
          </div>
          ...
          <form className="mt-[14px] flex flex-col gap-[17px]" onSubmit={submit}>
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              />
              {mismatch && (
                <p className="text-[13px] text-red-500">Passwords do not match.</p>
              )}
            </div>

            <button
              type="submit"
              disabled={busy || code.length !== 6 || password.length < 8 || mismatch}
