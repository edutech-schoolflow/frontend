      ...
      {draft.type === "theory" && (
        <div className="mb-4">
          <label className="mb-1 block text-[13px] font-medium text-text-body">
            Model answer{" "}
            <span className="font-normal text-[#9ca3af]">
              (optional — only visible to school admin)
            </span>
          </label>
          <textarea
            value={draft.answer}
...
export default function ExamPaperEditor({ paperId }: { paperId: string }) {
  ...
  const isEditable = paper?.status === "draft" || paper?.status === "rejected";
  ...
  const handleSubmit = async () => {
    ...
    const submitted = await submitExamPaper(paper.id);
    setPaper(submitted);
    setIsDirty(false);
  ...
  // Preview paper object with local edits merged in
  ...
  return (
    <div className="p-[30px]">
      ...
      {paper.status === "rejected" && paper.reviewComment && (
        <div className="mb-5 flex items-start gap-2.5 rounded-[10px] border border-[#fecaca] bg-[#fef2f2] px-4 py-3">
          <AlertCircle className="mt-0.5 h-[16px] w-[16px] shrink-0 text-[#dc2626]" />
          <div>
            <p className="text-[13px] font-medium text-[#dc2626]">
              Returned for revision
            </p>
            <p className="text-[13px] text-[#b91c1c]">{paper.reviewComment}</p>
          </div>
        </div>
      )}
      ...
      {paper.status === "submitted" && (
        <div className="mb-5 flex items-center gap-2.5 rounded-[10px] border border-[#bfdbfe] bg-[#eff6ff] px-4 py-3">
          <CheckCircle2 className="h-[16px] w-[16px] shrink-0 text-[#2563eb]" />
          <p className="text-[13px] text-[#1d4ed8]">
            Submitted for school approval. You will be notified when it is
            reviewed.
          </p>
        </div>
      )}
            ...
            <h2 className="text-[14px] font-semibold text-text-heading">
              ...
              {questions.length > 0 && (
                <span className="ml-2 text-[12px] font-normal text-[#9ca3af]">
                  {questions.length} total · {mcQuestions.length} MC ·{" "}
                  {theoryQuestions.length} theory · {totalMarks} marks
                </span>
              )}
            </h2>
          </div>
